package product.bean;

public class RegisterDto {
	
	private String p_content,p_code,p_name,p_price,p_img,p_people,p_invest,p_purpose,
				p_way,p_rate,p_term,p_funding,p_repayday,p_success,p_file,p_category;
	private int no;
	
	
	
	
	public String getP_category() {
		return p_category;
	}

	public void setP_category(String p_category) {
		this.p_category = p_category;
	}

	public String getP_name() {
		return p_name;
	}

	public void setP_name(String p_name) {
		this.p_name = p_name;
	}

	public String getP_price() {
		return p_price;
	}

	public void setP_price(String p_price) {
		this.p_price = p_price;
	}

	public String getP_img() {
		return p_img;
	}

	public void setP_img(String p_img) {
		this.p_img = p_img;
	}

	public String getP_people() {
		return p_people;
	}

	public void setP_people(String p_people) {
		this.p_people = p_people;
	}

	public String getP_invest() {
		return p_invest;
	}

	public void setP_invest(String p_invest) {
		this.p_invest = p_invest;
	}

	public String getP_purpose() {
		return p_purpose;
	}

	public void setP_purpose(String p_purpose) {
		this.p_purpose = p_purpose;
	}

	public String getP_way() {
		return p_way;
	}

	public void setP_way(String p_way) {
		this.p_way = p_way;
	}

	public String getP_rate() {
		return p_rate;
	}

	public void setP_rate(String p_rate) {
		this.p_rate = p_rate;
	}

	public String getP_term() {
		return p_term;
	}

	public void setP_term(String p_term) {
		this.p_term = p_term;
	}

	public String getP_funding() {
		return p_funding;
	}

	public void setP_funding(String p_funding) {
		this.p_funding = p_funding;
	}

	public String getP_repayday() {
		return p_repayday;
	}

	public void setP_repayday(String p_repayday) {
		this.p_repayday = p_repayday;
	}

	public String getP_success() {
		return p_success;
	}

	public void setP_success(String p_success) {
		this.p_success = p_success;
	}

	public String getP_file() {
		return p_file;
	}

	public void setP_file(String p_file) {
		this.p_file = p_file;
	}

	public int getNo() {
		return no;
	}

	public void setNo(int no) {
		this.no = no;
	}

	public String getP_content() {
		return p_content;
	}

	public void setP_content(String p_content) {
		this.p_content = p_content;
	}

	public String getP_code() {
		return p_code;
	}

	public void setP_code(String p_code) {
		this.p_code = p_code;
	}
	
	

}
